...
func TestExpireIncomplete_RealCapturePathProducesNoIncompleteUnits(t *testing.T) {
	...
	captureSvc := brain.NewCaptureService(fixedClock{now: now}, &counterIDs{}, units, embeddings, lexical, relations, decisions, llm, llm, llm, embed, brain.NewIndex(idx), signals, triggers, timers, 0.5, nil)
	...
	consolidateSvc := brain.NewConsolidateService(fixedClock{now: later}, cfg, units, relations, &counterIDs{}, decisions, recallSvc, fakeprovider.New(t, ""), sqlite.NewSelfModelRepo(v), sqlite.NewStateRepo(v), sqlite.NewPendingQuestionRepo(v))
