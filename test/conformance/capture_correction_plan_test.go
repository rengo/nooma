...
func TestCapture_CorrectionPlanWritesExactlyOneField(t *testing.T) {
	newSvc := func(t *testing.T, units *memrepo.Units, decisions *memrepo.DecisionLog, llmCase string) *brain.CaptureService {
		...
		return brain.NewCaptureService(fixedClock{now: fixedNow}, &counterIDs{}, units, embeddings, lexical, relations, decisions, llm, llm, llm, embed, brain.NewIndex(idx), memrepo.NewSignals(), memrepo.NewTriggers(), memrepo.NewTimers(), 0.5)
	}
	...
	t.Run("at most one Update* call reaches ports.UnitRepo per correction", func(t *testing.T) {
		...
		svc := brain.NewCaptureService(fixedClock{now: fixedNow}, &counterIDs{}, counting, embeddings, lexical, relations, decisions, llm, llm, llm, embed, brain.NewIndex(idx), memrepo.NewSignals(), memrepo.NewTriggers(), memrepo.NewTimers(), 0.5)
