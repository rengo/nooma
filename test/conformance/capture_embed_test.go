...
func TestCapture_EmbedsThePersistedUnitExactlyOnce(t *testing.T) {
	...
	svc := brain.NewCaptureService(fixedClock{now: now}, &counterIDs{}, units, embeddings, lexical, relations, decisions, llm, llm, embed, brain.NewIndex(initialIdx), memrepo.NewSignals())
...
func TestCapture_NoEmbeddingWrittenForAUnitNotPersisted(t *testing.T) {
	...
	svc := brain.NewCaptureService(fixedClock{now: now}, &counterIDs{}, units, embeddings, memrepo.NewLexical(), memrepo.NewRelations(), decisions, llm, llm, embed, brain.NewIndex(initialIdx), memrepo.NewSignals())
