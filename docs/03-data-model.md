...
- **Enums as TEXT**, disciplined by the application (never rigid CHECK-enums: vocabularies
  grow).
- **Timestamps**: TEXT ISO-8601 UTC (`YYYY-MM-DDTHH:MM:SSZ`). The user's timezone is **not** vault
  config — it travels inside the instant the clock returns, and reaches a prompt as that instant's
  UTC offset. `docs/02-cognitive-core.md` §5.1 carries the rule and why a config key would be a
  second source of truth for a fact the timestamp already holds.
