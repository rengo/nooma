...
- **Recurrence**: `recurrence_rule` (`yearly` | `monthly` | `daily` | `weekly`) +
  `recurrence_anchor` (`{month, day, weekday}`). **Which fields of the anchor a rule reads is
  the rule's own business**: `monthly` ignores the month, `weekly` reads only the weekday, and
  `daily` reads no anchor at all, because "every day" names no day.
  **The weekday is optional and its absence is meaningful.** Sunday is the zero value of a
  weekday in most calendars and in Go's own type, so a stated Sunday and an unstated weekday
  are only distinguishable if absence has its own representation. A weekly rule whose weekday
  never arrived falls back to the yearly reading of the date the user did state, rather than
  repeating on a Sunday nobody asked for. A daily occurrence is simply the next `recurrence_anchor_hour` strictly after the
...
| `proactive_check_interval` (`internal/scheduler.ProactiveCheckInterval`) — the delivery pass's cadence. **Also outside the gate's reach**, for the identical reason, and stated rather than implied. Bounded from both sides: longer, and an item deferred out of quiet hours waits past 07:00 by up to that long every morning; shorter, and a personal vault does arithmetic it has no reason to do | 5 — every 5 minutes |
