...
| `MergeProposals`: cosine exactly `BeliefMergeCosine` merges (boundary, both sides); the nearest existing belief wins; empty `existing` creates; a model mismatch surfaces `recall.ErrModelMismatch`; a zero vector surfaces `recall.ErrZeroVector`; un-normalized input still scores as cosine (normalization is internal) | L1 | `internal/core/consolidation/` | 4 |
