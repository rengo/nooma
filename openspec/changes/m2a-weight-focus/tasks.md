...
- [ ] **2a.1** Commit 1 (RED): `internal/core/weight/boost_test.go` — `Revive` strictly increasing
...
- [ ] **2a.2** Commit 2 (GREEN): implement `Revive` — `e := Effective(c.Weight, c.DecayRate,
...
- [ ] **2a.3** `docs/06-harness.md` §4: add I24's row — *"A weight write moves `weight` and
...
- [ ] **2a.4** doc 02 §2 amendment: replace "writes a new boosted weight" with the asymptotic
...
- [ ] **2a.5** §13: add `revive_gain` (0.35, `weight.ReviveGain`, chosen) and `weight_ceiling` (2.0,
      `weight.WeightCeiling`, chosen — pinned by R2.4/R2.7 once 2b lands) rows. Row count: 23 → 25.
      Verify: read the section.
      Requirement: design §5.1.
- [ ] **2a.6** Purity/coverage: `golangci-lint run`; `make cover`.
