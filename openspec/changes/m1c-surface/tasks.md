...
### C25 — post-merge: once `C24` sent the real prompt, a live `gpt-4o-mini` run found every `relation_evaluation` candidate's `id` spelled `outcome`'s own answer, and four of five cases were passing on that hint, not on judgment

**Found the same way `C24` was: a human ran `nooma doctor` against a real key**, this time
`gpt-4o-mini`, once `C24`'s fix made the gate send `brain.JudgePrompt`'s real ~un-stubbed text.
`relation_evaluation` scored 4/5, failing only `relation-no-match-for-dry-cleaning` —
`vocabulary failure — field "outcome" (unknown_enum)`. The model had answered
`{"outcome":"cand-unrelated"}`: it copied the candidate's own id verbatim.
`brain.JudgePrompt` renders `"  " + c.ID + ": " + c.Content` for every candidate, so
`cand-unrelated: Notes on PostgreSQL index types` sat in the prompt as the nearest string to "no
relation" anywhere in it — a stronger cue than the actual semantic mismatch the case exists to
test.

**The other four relation cases carried the identical construction, and three of them are false
positives, not passes.** Every candidate id in the corpus was hand-authored to describe the
scenario for a human reading the fixture: `cand-discard`, `cand-duplicate`, `cand-outage`,
`cand-related`. `outcome`'s own vocabulary is `new | duplicate | related`
(`internal/brain/capture.go`'s `JudgePrompt`) — copying `cand-duplicate`'s or `cand-related`'s own
id lands on a **legal** enum value, so those two cases graded clean whether or not the model
judged anything. Only `cand-unrelated` failed, because "unrelated" is not itself a member of
`outcome`'s vocabulary — the corpus's one visible failure was luck of which word happened to also
be illegal, not a difference in what the fixture was actually measuring. `cand-discard` and
`cand-outage` do not collide with `outcome`'s vocabulary at all (`discard` is `relation.Decide`'s
own threshold verdict, a different field than `outcome`; the outage case never reaches a JSON
verdict), so those two were not implicated by this specific mechanism, though they carry the same
authoring habit.

**Production never produces this shape.** Real candidate ids are `unit.ID`, generated by
`ports.IDGen` (`internal/ports`) — a live capture returns ids such as
`76429fab-28c8-443b-b853-421ee209274e`. No production identifier is chosen to describe its own
content, let alone to name the classification a downstream judge is being asked to produce. A
corpus id crafted for a fixture author's convenience is not equivalent in kind to a corpus id
production could plausibly generate — the first is an input a real system never sends, tested only
because it was cheap to write by hand.

**Investigated but not the same defect, and left unchanged: `capture_processing` (`classify`)
cases can carry a domain word that matches their own `type`.**
`classify-timer-set-a-timer.json`'s `message` — `"set a timer for 10 minutes"` — literally
contains `timer`, which is also `classify.KindTimer`'s own vocabulary value. Every other
`classify`-tagged case's `message` and every `orthogonalFields()` vocabulary word
(`internal/core/classify/outcomes.go`) was checked by hand against every case's `message`; no
other collision was found. This is not the same defect as the relation candidate ids, for a
structural reason rather than a stylistic one: a candidate id is a label this codebase's own
storage layer invents, free to be anything, and a fixture author's choice to make it descriptive
is purely for human convenience — exactly the shape `C24`'s own "written once, used in two places"
lesson warns about. A `message` field is the thing being classified; a real user asking to set a
timer says the word "timer" in production too, and a model using that literal cue is doing the
job correctly, not gaming a fixture. Changing `classify-timer-set-a-timer.json` would make it a
worse recording of production, not a more honest one — left as-is.

**Nothing else was found reaching a live prompt.** `qualityGatePrompt`
(`cmd/nooma/doctor.go`) builds `capture_processing`'s prompt from `c.Message` alone (plus a `nil`
beliefs slice and the injected clock, neither sourced from the corpus) and `relation_evaluation`'s
from `c.Message` and `c.Candidates[].{ID,Content}` alone. A case's own `id` (its filename) is
never passed to either builder — confirmed by reading `qualityGatePrompt`, not assumed — so a
descriptive case id such as `relation-no-match-for-dry-cleaning` is harmless; only a *candidate's*
id reaches a rendered prompt. `provider`, `model`, `task`, `response`, and `error` are not even
unmarshaled into `llm.Case` (`testdata/llm/corpus.go`'s comment states this by design), so none of
them can leak into a live send either.

**Resolution.** The five `testdata/llm/cases/relation-*.json` files' candidate `id` (and each
file's own `response.target_unit_id`, which must keep naming its file's candidate) are replaced
with realistic v4 UUIDs, matching the shape a real `ports.IDGen` produces. `test/conformance/capture_relation_judge_test.go`'s `judgeTestFixture` call sites are updated to the
same literals, so the seeded pool id and the persisted `ToUnitID` assertions stay byte-identical to
what each case's `response` now says. With the ids neutral, all five relation cases still decode
clean under `TestRunLLMQualityCheck`'s scripted-provider suite (spec R5.5: no test in this
codebase calls a real provider) — a live re-run against an actual model, which would confirm
whether the four previously-passing cases keep passing on judgment rather than on the hint, is
outside what any test in this repository can do and is left to whoever next runs `nooma doctor`
against a live key.

**Regression coverage added**: `TestRelationCandidateIDsDoNotLeakTheOutcomeVocabulary`
(`cmd/nooma/doctor_test.go`) fails red against the pre-fix corpus (four candidate ids match
`outcome`'s own vocabulary or its `unrelated` negation) and green after. Its own stated limit: it
proves no candidate id contains `new`/`duplicate`/`related`/`unrelated` as a substring — it cannot
prove a case is measuring judgment rather than pattern-matching in general (only a live provider
run, outside every test's own network ban, can), and it does not extend to `capture_processing`'s
`message` field, which this entry argues by construction should not be constrained the same way.
