...
| L2 `test/conformance` | `TestHTTPAPISecretCompareStructure` (`httpapi_secret_compare_test.go`) — an AST-level gate, not a response-level one, proving two things: (1) `cookie.go`'s `presentedSecret` — the decode step — returns a single `[]byte`, no `error`, no `http.ResponseWriter`, so an early return on a decode error has no signature left to be written in; (2) `requireCookie`'s and `requireToken`'s own handler bodies — the closure each returns, and the per-request `http.HandlerFunc` literal that closure returns — match a literal, declared template exactly, statement by statement, in order, including a dedicated check that the compare `if`'s condition IS (not merely reaches) a `subtle.ConstantTimeCompare(...) != 1` comparison | `presentedSecret` grows an `error` result or an `http.ResponseWriter` parameter; any deviation from either template — an extra or missing statement (a busy loop, a duplicated `r.Cookie`/decode check with its own early return), a condition joined by `&&`/`||`, `subtle.ConstantTimeCompare` replaced by hand-rolled equality, the handler wrapped by another call instead of returned directly, or `requireCookie`/`requireToken` renamed or removed (the gate is vacuity-guarded against finding NOTHING to check — zero functions in the package, or the checked name absent). **Does not** flag a change made to both the code and the matching template in the same commit — that is the template's whole point, stated in its own failure message — nor anything confined to `presentedSecret`'s own body beyond its signature (a busy loop there stays a reviewed property, not an enforced one). **Inversion, stated explicitly**: an earlier version of this gate was required to stay sound under in-package helper extraction in either direction (moving the compare or the decode into a helper had to PASS); this template-based version requires the opposite — such a move now FAILS, because it changes the pinned statement sequence, by design (`sdd/m4a-ui-foundation/apply-progress`). **Round 5 correction**: "vacuity-guarded — it fails loudly rather than finding nothing to check" was true only for finding nothing; it said nothing about finding the WRONG thing. `parsePackageFuncDecls` resolved `requireCookie`/`requireToken`/`presentedSecret` by listing every non-`_test.go` file and letting `funcs[name] = decl` overwrite with no duplicate check — `go/parser` does not evaluate build constraints, so a second declaration of a checked name in another file (a decoy shadowing a backdoored real file, or a build-tag-disabled real file shadowing a shipped backdoor, in either direction) silently substituted with no signal either way. It now `t.Fatalf`s, naming both files and the name, the moment a checked name is declared twice, and does not attempt to evaluate build constraints to decide which declaration is real — a legitimate same-name GOOS split needs its own explicit, reviewed allowance, not a guess |
| L2 `test/conformance` | `TestUIMuxWiringMatchesDeclaredGuardTable` (`httpapi_ui_wiring_test.go`) — the structural gate `TestHTTPAPISecretCompareStructure` does not provide: that `newUIMux`'s registrations actually use the guard the gate above inspects, not merely that a function BY THAT NAME exists somewhere. Parses `internal/httpapi/server.go`'s `newUIMux`, resolves each `mux.Handle(pattern, handler)` call's handler expression (through one level of local-variable assignment, e.g. `guardedUI := requireCookie(d.Token)(d.UI)`), and matches the result against a declared table: `GET /ui` and `GET /ui/{$}` must each resolve to exactly `requireCookie(d.Token)(d.UI)`; the three `/ui/static/*` leaves must resolve to anything else. Round 5, judge B: added a sibling `weakerUIGuard(d.Token)`, wired it into `GET /ui/{$}` in place of `requireCookie`, left `requireCookie` itself untouched — every existing test, including the one below, stayed green, because nothing had ever checked that the guarded routes use the guard the structural gate inspects | A guard function swapped for another one on a guarded leaf (round 5, judge B's exact exploit — `GET /ui/{$}` wired to `weakerUIGuard(d.Token)` instead); a guarded leaf registered unwrapped (`mux.Handle("GET /ui/{$}", d.UI)`); an open leaf wrongly wrapped (any leaf not in the table — e.g. a `/ui/login` added and guarded — reported as an undeclared pattern, guarded or not, since this table does not yet include PR 4b's `/ui/login`; adding it there is this gate's own extension point when PR 4b lands); a third guarded view added to `newUIMux` without a matching table row, whether or not it is actually wrapped. **Scoped to the UI subtree only**: the API side's `apiRoutes` (design D10) is guarded once, for the whole `guardedMux`, by a single `requireToken(d.Token)(guardedMux)` wrap around the entire mux in `Handler` — there is no per-route wiring on that side for a route to omit, so the per-leaf check this gate performs does not apply there; a route added to `apiRoutes` is guarded by construction, already proven by `TestGuardedRoutesRequireToken` iterating the same slice `Handler` registers from |
...
| L1 `httpapi` | `TestLoginSubmitBodyIsBounded` — a well-formed but wrong `token` field over 4096 bytes → 400, no `Set-Cookie`, the response body carries neither the submitted value nor the real token — proving the bound (§3.3's `http.MaxBytesReader(w, r.Body, 4096)`) fires before the comparison is ever reached | The `MaxBytesReader` line removed (`ParseForm` then succeeds, the wrong token reaches the comparison, 401 instead of 400); its limit widened past the body this test sends (same 401-instead-of-400 tell) |
