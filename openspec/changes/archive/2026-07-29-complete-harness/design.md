...
| `foreign_keys` | `on` | Doc 03 conventions. It is **per connection**. Upstream SQLite defaults it to OFF, but this driver does not: `ncruces/go-sqlite3-wasm` compiles with `SQLITE_DEFAULT_FOREIGN_KEYS 1` (`build/sqlite_opt.h:23`), so a bare connection already reads `1` — verified both in the vendored source and empirically. It is still requested explicitly, because a vendored compile flag is not a contract this project controls and can change under a driver upgrade. `relations`, `unit_embeddings`, `self_beliefs.source_unit_id` and `measurements.ref_unit_id` all carry `ON DELETE` behaviour that simply does not exist on a connection where this is off |
...
| **R10** | **F1**: `units.rowid` is unstable across `VACUUM` while `units_fts` is keyed on it | Not fixed here. Recorded in doc 03 by PR 4; recommended for an ADR before `nooma export` lands. **Measured, slice 4a**: `VACUUM`/`VACUUM INTO` were probed across the Go driver, the `sqlite3` CLI, and Python's `sqlite3` module, with rowid gaps present — every run preserved the existing rowids rather than renumbering. SQLite's docs still say a vacuum "may renumber" them, so the hazard remains real in principle and doc 03's unconditional `INSERT INTO units_fts(units_fts) VALUES('rebuild')` guidance is unchanged; this row now records that the risk was measured, not only read off the docs |
